...
load("/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/OccupancyModeling/Florida/ProbOcc18s_dataOUT14_32_30_FK.Rdata")
...
(READcount0<-sum(sum(FK_18S@otu_table)))
(OTUcount0<-nrow(FK_18S@otu_table))

FK_18S_prob <- subset(dataOUT,probOcc >0.8)
FK_18S_unique_OTUs <- as.character(unique(FK_18S_prob$OTUname))
FK_18S_pruned <- prune_taxa(FK_18S_unique_OTUs, FK_18S)

#pdf(file = "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/OccupancyModeling/Florida/hist_18S_prob.pdf")
#hist(dataOUT$probOcc)
#dev.off()

allOTUs<-as.data.frame(otu_table(FK_18S_pruned, taxa_are_rows=TRUE))
(READcount1<-sum(sum(allOTUs)))
(OTUcount1<-nrow(allOTUs))
Meta<-as.data.frame(FK_18S_pruned@sam_data)
...
###################################################
samples_all <- allOTUs
CONTROL<-Meta[which(grepl("blank", Meta$sample_type)),]
...
############write out
library(magrittr)
DECONTAM_biom_FK_18S<-prune_taxa(row.names(DECONTAM), FK_18S_pruned)%>%
  prune_samples(colnames(DECONTAM), .)

sample_names(DECONTAM_biom_FK_18S)
FK_18S_m2w <- subset_samples(DECONTAM_biom_FK_18S, description == "m2w")
sample_names(FK_18S_m2w)

write.csv(FK_18S_m2w@otu_table, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/FK/18S/FK_18S_otu_table.csv")  #is this the best way to save biom files?
write.csv(FK_18S_m2w@tax_table, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/FK/18S/FK_18S_tax_table.csv")  #is this the best way to save biom files?
write.csv(FK_18S_m2w@sam_data, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/FK/18S/FK_18S_sam_table.csv")  #is this the best way to save biom files?


#DECONTAM<-data.frame(DECONTAM, allOTUs[match(row.names(DECONTAM), row.names(allOTUs)),64:65])
#write.csv(DECONTAM, "M2W_DECONTAM_18S_FK_20171101.csv")

SummaryTableOut_FK_18S<-t(data.frame(c(READcount0, OTUcount0),c(READcount1, OTUcount1),c(READcount2, OTUcount2),c(READcount3, OTUcount3)))
colnames(SummaryTableOut_FK_18S)<-c("Reads","OTUs")
row.names(SummaryTableOut_FK_18S)<-c("Raw", "PostOccupancy","PostSubtraction","final")


write.csv(SummaryTableOut_FK_18S, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/FK/18S/FK_18S_summary.csv")  #is this the best way to save biom files?
