...
load("/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/OccupancyModeling/Florida/ProbOccCOI_dataOUT14_53_45_FK.Rdata")
...
(READcount0<-sum(sum(FK_COI@otu_table)))
(OTUcount0<-nrow(FK_COI@otu_table))

FK_COI_prob <- subset(dataOUT,probOcc >0.8)
FK_COI_unique_OTUs <- as.character(unique(FK_COI_prob$OTUname))
FK_COI_pruned <- prune_taxa(FK_COI_unique_OTUs, FK_COI)

#pdf(file = "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/OccupancyModeling/Florida/hist_COI_prob.pdf")
#hist(dataOUT$probOcc)
#dev.off()

allOTUs<-as.data.frame(otu_table(FK_COI_pruned, taxa_are_rows=TRUE))
(READcount1<-sum(sum(allOTUs)))
(OTUcount1<-nrow(allOTUs))
Meta<-as.data.frame(FK_COI_pruned@sam_data)
...
############write out
library(magrittr)
DECONTAM_biom_FK_COI<-prune_taxa(row.names(DECONTAM), FK_COI_pruned)%>%
  prune_samples(colnames(DECONTAM), .)

sample_names(DECONTAM_biom_FK_COI)
FK_COI_m2w <- subset_samples(DECONTAM_biom_FK_COI, description == "m2w")
sample_names(FK_COI_m2w)

write.csv(FK_COI_m2w@otu_table, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/FK/COI/FK_COI_otu_table.csv")  #is this the best way to save biom files?
write.csv(FK_COI_m2w@tax_table, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/FK/COI/FK_COI_tax_table.csv")  #is this the best way to save biom files?
write.csv(FK_COI_m2w@sam_data, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/FK/COI/FK_COI_sam_table.csv")  #is this the best way to save biom files?

#DECONTAM<-data.frame(DECONTAM, allOTUs[match(row.names(DECONTAM), row.names(allOTUs)),64:65])
#write.csv(DECONTAM, "M2W_DECONTAM_COI_FK_20171101.csv")

SummaryTableOut_FK_COI<-t(data.frame(c(READcount0, OTUcount0),c(READcount1, OTUcount1),c(READcount2, OTUcount2),c(READcount3, OTUcount3)))
colnames(SummaryTableOut_FK_COI)<-c("Reads","OTUs")
row.names(SummaryTableOut_FK_COI)<-c("Raw", "PostOccupancy","PostSubtraction","final")

write.csv(SummaryTableOut_FK_COI, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/FK/COI/FK_COI_summary.csv")  #is this the best way to save biom files?
