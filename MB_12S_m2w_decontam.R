...
load("/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/OccupancyModeling/Monterey/ProbOcc12s_dataOUT11_25_26.Rdata")
...
(READcount0<-sum(sum(MB_12S@otu_table)))
(OTUcount0<-nrow(MB_12S@otu_table))

MB_12S_prob <- subset(dataOUT,probOcc >0.8)
MB_12S_unique_OTUs <- as.character(unique(MB_12S_prob$OTUname))
MB_12S_pruned <- prune_taxa(MB_12S_unique_OTUs, MB_12S)

#a <- as.data.frame(MB_12S_pruned@otu_table)

allOTUs<-as.data.frame(otu_table(MB_12S_pruned, taxa_are_rows=TRUE))
(READcount1<-sum(sum(allOTUs)))
(OTUcount1<-nrow(allOTUs))
Meta<-as.data.frame(MB_12S_pruned@sam_data)
...
############write out
library(magrittr)
DECONTAM_biom_MB_12S<-prune_taxa(row.names(DECONTAM), MB_12S_pruned)%>%
  prune_samples(colnames(DECONTAM), .)

sample_names(DECONTAM_biom_MB_12S)
MB_12S_m2w <- subset_samples(DECONTAM_biom_MB_12S, description == "m2w")
sample_names(MB_12S_m2w)

write.csv(MB_12S_m2w@otu_table, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/12S/MB_12S_otu_table.csv")  #is this the best way to save biom files?
write.csv(MB_12S_m2w@tax_table, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/12S/MB_12S_taxa_table.csv")  #is this the best way to save biom files?
write.csv(MB_12S_m2w@sam_data, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/12S/MB_12S_sam_data.csv")  #is this the best way to save biom files?
...
SummaryTableOut_MB_12S<-t(data.frame(c(READcount0, OTUcount0),c(READcount1, OTUcount1),c(READcount2, OTUcount2),c(READcount3, OTUcount3)))
colnames(SummaryTableOut_MB_12S)<-c("Reads","OTUs")
row.names(SummaryTableOut_MB_12S)<-c("Raw", "PostOccupancy","PostSubtraction","final")

write.csv(SummaryTableOut_MB_12S, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/12S/MB_12S_summary.csv")  #is this the best way to save biom files?
