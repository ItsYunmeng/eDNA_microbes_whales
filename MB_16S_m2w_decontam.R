...
load("/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/OccupancyModeling/Monterey/ProbOcc16s_dataOUT14_21_01_MB.Rdata")
MB_16S <- merge_phyloseq(MB_16S_biom@otu_table, MB_16S_biom@tax_table,sample_data(read.csv("/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/metadata/m2w_16S_MB_metadata.csv", row.names=2)))
  (READcount0<-sum(sum(MB_16S@otu_table)))
  (OTUcount0<-nrow(MB_16S@otu_table))

hist(dataOUT$probOcc)

MB_16S_prob <- subset(dataOUT,probOcc >0.8)
MB_16S_unique_OTUs <- as.character(unique(MB_16S_prob$OTUname))
MB_16S_pruned <- prune_taxa(MB_16S_unique_OTUs, MB_16S)

a <- as.data.frame(MB_16S@tax_table)
#MB_16S_unique_OTUs

allOTUs<-as.data.frame(otu_table(MB_16S_pruned, taxa_are_rows=TRUE))
(READcount1<-sum(sum(allOTUs)))
(OTUcount1<-nrow(allOTUs))
Meta<-as.data.frame(MB_16S_pruned@sam_data)
...
############write out
library(magrittr)
DECONTAM_biom_MB_16S<-prune_taxa(row.names(DECONTAM), MB_16S_pruned)%>%
  prune_samples(colnames(DECONTAM), .)

sample_names(DECONTAM_biom_MB_16S)
MB_16S_m2w <- subset_samples(DECONTAM_biom_MB_16S, description == "m2w")
sample_names(MB_16S_m2w)

write.csv(MB_16S_m2w@otu_table, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/16S/MB_16S_otu_table.csv")  #is this the best way to save biom files?
write.csv(MB_16S_m2w@tax_table, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/16S/MB_16S_tax_table.csv")  #is this the best way to save biom files?
write.csv(MB_16S_m2w@sam_data, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/16S/MB_16S_sam_table.csv")  #is this the best way to save biom files?
...
SummaryTableOut_MB_16S<-t(data.frame(c(READcount0, OTUcount0),c(READcount1, OTUcount1),c(READcount2, OTUcount2),c(READcount3, OTUcount3)))
colnames(SummaryTableOut_MB_16S)<-c("Reads","OTUs")
row.names(SummaryTableOut_MB_16S)<-c("Raw", "PostOccupancy","PostSubtraction","final")

write.csv(SummaryTableOut_MB_16S, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/16S/MB_16S_summary.csv")  #is this the best way to save biom files?
