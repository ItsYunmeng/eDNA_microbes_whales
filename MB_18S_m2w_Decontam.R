...
load("/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/OccupancyModeling/Monterey/ProbOcc18s_dataOUT14_35_57_MB.Rdata")
...
(READcount0<-sum(sum(MB_18S@otu_table)))
(OTUcount0<-nrow(MB_18S@otu_table))

MB_18S_prob <- subset(dataOUT,probOcc >0.8)
MB_18S_unique_OTUs <- as.character(unique(MB_18S_prob$OTUname))
MB_18S_pruned <- prune_taxa(MB_18S_unique_OTUs, MB_18S)


allOTUs<-as.data.frame(otu_table(MB_18S_pruned, taxa_are_rows=TRUE))
	(READcount1<-sum(sum(allOTUs)))
	(OTUcount1<-nrow(allOTUs))
Meta<-as.data.frame(MB_18S_pruned@sam_data)
...
############write out
library(magrittr)
DECONTAM_biom_MB_18S<-prune_taxa(row.names(DECONTAM), MB_18S_pruned)%>%
  prune_samples(colnames(DECONTAM), .)

sample_names(DECONTAM_biom_MB_18S)
MB_18S_m2w <- subset_samples(DECONTAM_biom_MB_18S, description == "m2w")
sample_names(MB_18S_m2w)

write.csv(MB_18S_m2w@otu_table, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/18S/MB_18S_otu_table.csv")
write.csv(MB_18S_m2w@tax_table, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/18S/MB_18S_taxa_table.csv")
write.csv(MB_18S_m2w@sam_data, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/18S/MB_18S_sam_table.csv")
...
SummaryTableOut_MB_18S<-t(data.frame(c(READcount0, OTUcount0),c(READcount1, OTUcount1),c(READcount2, OTUcount2),c(READcount3, OTUcount3)))
	colnames(SummaryTableOut_MB_18S)<-c("Reads","OTUs")
	row.names(SummaryTableOut_MB_18S)<-c("Raw", "PostOccupancy","PostSubtraction","final")

#write summary table
write.csv(SummaryTableOut_MB_18S, "/Users/anni_djurhuus/Documents/Projects/MBON/m2w/google_drive/data/MEGAN6/post_decont/MB/18S/MB_18S_summary.csv")  
