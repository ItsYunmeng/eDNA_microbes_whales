...
#a <- p.adjust(moduleTraitPvalue_MB, method="bonferroni")
...
par(mar=c(3,4,1,4))
barplot(rich_all_mod, col=color, cex.axis=0.8,cex.lab=0.8, space=c(0.5,2,2,1,3,3,0,2),cex=0.8, ylab="Number of taxa per module", xlab="", xaxt="n",ylim=c(0,125))
...
par(new=TRUE)
plot(env_MB$Mean.SST~dmy(env_MB$Date),type="l",ylim=c(12,18), lwd=1,xlab="", ylab="",yaxt="n",xaxt="n",col=alpha("grey40"), lty=2)
...
cyt = exportNetworkToCytoscape(modTOM,
      ...
      weighted = TRUE,threshold = 0, nodeNames = modProbes, nodeAttr = moduleColors_MB[inModule])
...
cyt_dg = exportNetworkToCytoscape(modTOM_dg,
      edgeFile = paste("CytoscapeInput-edges-", paste(blue, collapse="-"), ".txt", sep=""),
      nodeFile = paste("CytoscapeInput-nodes-", paste(blue, collapse="-"), ".txt", sep=""),
      weighted = TRUE,threshold = 0, nodeNames = modProbes_dg, nodeAttr = moduleColors_MB[inModule_dg])
...
labels_blue <- c("Detritivores (11.0%)", "Autotrophs (33.3%)", "Mixotrophs (9.8%)", "Heterotrophs (44.4%)","Tertiary consumers (1.2%)")

trophic_grey <- c(26, 20, 12, 55, 1)
trophic_grey <- prop.table(trophic_grey)*100
labels_grey <- c("Bacteria/Archaea/Fungi (22.8%)", "Primary producers (17.5%)", "Primary consumers (10.5%)", "Secondary consumers (48.2%)","Tertiary consumers (0.9%)")
...
lines(newblue, pre_blue[,1], lty=1,col="grey50")
...
#Scombridae <- cbind(data.frame(Abundance=MB_merge_filt_t$`f:Scombridae`),data.frame(Date=MB_merge_filt_t$collection_date), Taxa=(c("Scombridae")))
...
Pelagibacteracea <- cbind(data.frame(Abundance=MB_merge_filt_t$`f:Pelagibacteracea`),data.frame(Date=MB_merge_filt_t$collection_date), Taxa=(c("Pelagibacteracea")))

data_ggridges <- rbind(Otariidae,Carangidae,Temoridae,Syndiniaceae,Pelagibacteracea)
...
data_ggridges$Abundance <- data_ggridges$Abundance*100
...
ggplot(Data_sum, aes(x = dayVector, y = Taxa,fill=Taxa)) + 
  geom_density_ridges(scale=1.15, alpha=0.7, color="grey80",jittered_points = TRUE,
...
Corycaeidae <- cbind(data.frame(Abundance=MB_merge_filt_t$`f:Corycaeidae`),data.frame(Date=MB_merge_filt_t$collection_date), Taxa=(c("Corycaeidae")))
Katablepharidaceae <- cbind(data.frame(Abundance=MB_merge_filt_t$`f:Katablepharidaceae`),data.frame(Date=MB_merge_filt_t$collection_date), Taxa=(c("Katablepharidaceae")))
Narcomedusae <- cbind(data.frame(Abundance=MB_merge_filt_t$`o:Narcomedusae`),data.frame(Date=MB_merge_filt_t$collection_date), Taxa=(c("Narcomedusae")))
Opheliidae <- cbind(data.frame(Abundance=MB_merge_filt_t$`f:Opheliidae`),data.frame(Date=MB_merge_filt_t$collection_date), Taxa=(c("Opheliidae")))
#Engraulidae <- cbind(data.frame(Abundance=MB_merge_filt_t$`f:Engraulidae`),data.frame(Date=MB_merge_filt_t$collection_date), Taxa=(c("Engraulidae")))
...
data_ggridges_b <- rbind(Balaenopteridae,Euphausiidae,Katablepharidaceae,Narcomedusae,Opheliidae,Corycaeidae)
...
data_ggridges_b$Abundance <- data_ggridges_b$Abundance*100
...
ggplot(Data_sum_b, aes(x = dayVector, y = Taxa,fill=Taxa)) + 
  geom_density_ridges(scale=1.15, alpha=0.6,color="grey80",jittered_points = TRUE,
    position = position_points_jitter(width = 0.05, height = 0),
    point_shape = '|', point_size = 3, point_alpha = 1, alpha = 0.7) +   
  scale_fill_manual(values = c("#00aedb", "lightblue1","#00aedb","lightblue1","#00aedb","lightblue1"), guide = "none") +
  theme_ridges(grid = FALSE, center = TRUE)+
  xlab("Days since April 30 2015") + ylab("")
dev.off()
