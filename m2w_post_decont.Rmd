...
library(here); library(plyr); library(biomformat); library(tidyverse); library(phyloseq);  library(vegan); library(reshape2); library(MASS); library(stringr); library(purrr); library(ggplot2); library(Hmisc); library(lubridate); library(corrplot)
```
...
badTaxa_FK = c("f:no hits", "f:Bovidae", "f:not assigned", "o:Diptera", "f:Hominidae", "f:Insecta")
...
badTaxa_MB = c("f:no hits", "f:Bovidae", "f:not assigned", "o:Diptera","f:Hominidae")
...
par(new=TRUE)
plot(env_FK$Anomaly~dmy(env_FK$Date),type="l",ylim=c(0,5), lwd=5,xlab="", ylab="",yaxt="n",xaxt="n",col=alpha("grey",0.5))
legend("topleft", "A", bty="n")
#axis(side=4)
...
par(new=TRUE)
plot(env_MB$Anomaly~dmy(env_MB$Date),type="l",ylim=c(0,5), lwd=5,xlab="", ylab="",yaxt="n",xaxt="n",col=alpha("grey",0.5))
axis(side=4)
mtext("Sea surface anomaly",side=4,line=1.9,las=3)
legend("topleft", "B", bty="n")
```

```{r plots of correlations, echo=FALSE, fig.cap="\\label{fig:fig 2}Figure 3: Correlation of the top 100 taxa for Florida Keys (top) and Monterey Bay (bottom)  only including significant values of a p-vallue equal to or less than 0.05. These correlation matrices were performed on relative abundance data and ordered by hierarcical clustering on dissimilarities between samples.", fig.height=17, fig.width=10, message=FALSE, warning=FALSE}

par(mfrow=c(2,1))
top_50_merge_FK <- prune_taxa(names(sort(taxa_sums(FK_all_ps_merge),
                                         TRUE)[1:100]),FK_all_ps_merge)
cor_FK <- as.matrix(top_50_merge_FK@otu_table)
correlation_FK <-cor(cor_FK)
res2_FK<-rcorr(cor_FK)
#flattenCorrMatrix(res2$r, res2$P)
corrplot(res2_FK$r, type="upper", order="hclust", 
         p.mat = res2_FK$P, sig.level = 0.05, tl.srt=45,tl.col="black",
         insig = "blank",tl.cex=0.4,diag=FALSE)

top_50_merge_MB <- prune_taxa(names(sort(taxa_sums(MB_all_ps_merge),
                                         TRUE)[1:100]),MB_all_ps_merge)
cor_MB <- as.matrix(top_50_merge_MB@otu_table)
correlation_MB <-cor(cor_MB)
res2_MB<-rcorr(cor_MB)
#flattenCorrMatrix(res2$r, res2$P)
corrplot(res2_MB$r, type="lower", order="hclust", 
         p.mat = res2_MB$P, sig.level = 0.05, tl.srt=45,tl.col="black",
         insig = "blank",tl.cex=0.4,diag=FALSE)

```


```{r plots and shit, echo=FALSE, fig.cap="\\label{fig:fig 3}Figure 4: Some of the most abundant organisms with the highest correlations from FKNMS (left) and MBNS (right).", fig.height=7, fig.width=8, message=FALSE, warning=FALSE}
...
#lines(FK_all_merge$f.Labridae/max(FK_all_merge$f.Labridae)~FK_all_merge$collection_date,lwd=1.5, col="grey")
...
legend("topleft",legend=c("Oithonidae", "Telonemida","Centropagidae", "Labridae"),col=c("darkolivegreen3","#009E73","darkolivegreen", "grey"),ncol=1, bty="n", lty=c(1,1,1), cex=0.6)
############ MB #######################
...
all_data <- merge_phyloseq(MB_all_tax_tf,FK_all_tax_tf)
...
```{r ordination plot of all data,echo=FALSE, fig.cap="\\label{fig:fig 6}Figure 6: Ordination on Jaccard distances for the complete dataset for both locations. Orange is Florida, blue is Monterey. This figure is not for publishing.", fig.height=4, fig.width=4, message=FALSE, warning=FALSE}
...
PA<-function(OTUs, DATES, REFERENCE = 1, METHOD = "euclidian"){
  ...
  dist.res<-sapply(X=unique(DATES), FUN=distFUN)
  dist.res<-as.data.frame(t(plyr::ldply(dist.res, rbind)))
  colnames(dist.res)<-unique(dayVector)
...
```{r pseudo autocorrelation, eval=FALSE, include=FALSE}
OTUs_16S_FK<-otu_table(FK_16S_fam)
dates_16S_FK<-dmy(sample_data(FK_16S_fam)$collection_date)  #
OTUs_18S_FK<-otu_table(FK_18S_fam)
dates_18S_FK<-ymd(sample_data(FK_18S_fam)$collection_date)  #
OTUs_COI_FK<-otu_table(FK_COI_fam)
dates_COI_FK<-ymd(sample_data(FK_COI_fam)$collection_date)  #
...
par(mfrow=c(1,3))
boxplot(dist.res_16S_FK, xlab="Elapsed Time in Days", ylab="Euclidian Distance", ylim=c(0,1),col="deepskyblue3")
boxplot(dist.res_18S_FK, xlab="Elapsed Time in Days", ylab="Euclidian Distance", ylim=c(0,1),col="deepskyblue3")
boxplot(dist.res_COI_FK, xlab="Elapsed Time in Days", ylab="Euclidian Distance", ylim=c(0,1),col="deepskyblue3")
...
par(mfrow=c(1,4))
boxplot(dist.res_16S_MB, xlab="Elapsed Time in Days", ylab="Euclidian Distance", ylim=c(0,1),col="deepskyblue3")
boxplot(dist.res_18S_MB, xlab="Elapsed Time in Days", ylab="Euclidian Distance", ylim=c(0,1),col="deepskyblue3")
boxplot(dist.res_COI_MB, xlab="Elapsed Time in Days", ylab="Euclidian Distance", ylim=c(0,1),col="deepskyblue3")
boxplot(dist.res_12S_MB, xlab="Elapsed Time in Days", ylab="Euclidian Distance", ylim=c(0,1),col="deepskyblue3")

```

```{r total taxa pseudo-autocorrelation}
