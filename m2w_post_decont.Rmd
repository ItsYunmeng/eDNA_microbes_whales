...
```{r pseudo autocorrelation FK, echo=FALSE, fig.cap="\\label{fig:fig 3}Figure 3: Pseudo-autocorrelation of FK reference point 1 (left) and with a moving reference point (right).", fig.height=5, fig.width=6.5, message=FALSE, warning=FALSE}
...
```{r pseudo autocorrelation MB, echo=FALSE, fig.cap="\\label{fig:fig 4}Figure 4: Pseudo-autocorrelation of MB with reference point 1 (left) and moving reference point (right).", fig.height=6.5, fig.width=6.5, message=FALSE, warning=FALSE}
############################### MB ########################
...
```{r plots of correlations, echo=FALSE, fig.cap="\\label{fig:fig 5}Figure 5: Correlation of the top 100 taxa for Florida Keys (top) and Monterey Bay (bottom)  only including significant values of a p-value equal to or less than 0.05. These correlation matrices were performed on relative abundance data and ordered by hierarcical clustering on dissimilarities between taxa", fig.height=17, fig.width=10, message=FALSE, warning=FALSE}
...
```{r dendrogram of top taxa, eval=FALSE, fig.cap="\\label{fig:fig 6}Figure 6: Dendrogram of the top 100 taxa for FK top and MB bottom.", fig.height=6, fig.width=10, message=FALSE, warning=FALSE, include=FALSE}
...
```{r heatmap of top taxa, echo=FALSE, fig.cap="\\label{fig:fig 6}Figure 6: Dendrogram of the top 50 taxa for FK left and MB right", fig.height=8, fig.width=10, message=FALSE, warning=FALSE}
...
#heatmap_FK <- heatmap.2(t(data.frame(top_taxa_merge_FK@otu_table))[idx_FK,],
#          trace="none", col=hmcol,cexRow=0.5,cexCol=0.5,distfun = function(x) dist(x,method = 'euclidean'))

#rv_MB <- rowVars(t(data.frame(top_taxa_merge_MB@otu_table)))
#idx_MB <- order(-rv_MB)[1:50]
#heatmap_MB <- heatmap.2(t(data.frame(top_taxa_merge_MB@otu_table))[idx_MB,],
#          trace="none",col=hmcol,cexRow=0.5,cexCol=0.5,distfun = function(x) dist(x,method = 'euclidean'))
...
library(colorspace)
library(GetoptLong)
top_30_FK <- prune_taxa(names(sort(taxa_sums(FK_all_tax_ps_ts_merge),
         TRUE)[1:30]),FK_all_tax_ps_ts_merge)
top_30_MB <- prune_taxa(names(sort(taxa_sums(MB_all_tax_ps_ts_merge),
         TRUE)[1:30]),MB_all_tax_ps_ts_merge)

pal <- colorRampPalette(brewer.pal(11, "RdYlGn"))(100)

pushViewport(viewport(x = 0, width = 0.5, just = "left"))
draw(Heatmap(t(data.frame(top_30_MB@otu_table)),col=pal,name = "MB"),k=3, newpage = FALSE)
popViewport()

pushViewport(viewport(x = 1, width = 0.5, just = "right"))
draw(Heatmap(t(data.frame(top_30_FK@otu_table)),col=pal,name = "FK"),k=4, newpage = FALSE)
popViewport()
```


```{r plots and shit, echo=FALSE, fig.cap="\\label{fig:fig 7}Figure 7: Some of the most abundant organisms with the highest correlations from FKNMS (left) and MBNS (right).", fig.height=5.5, fig.width=6, message=FALSE, warning=FALSE}
...
```{r plot ordinations, echo=FALSE, fig.cap="\\label{fig:fig 8}Figure 8: Some of the most abundant organisms with the highest correlations from the Florida Keys National Marine Sanctuary (A) and Monterey Bay National Marine Sanctuary (B). Colored by replicate with point type representing year (2015=square, 2016=circle).", fig.height=4, fig.width=6, message=FALSE, warning=FALSE}
par(mfrow=c(1,2))
par(mar=c(4,5,2,0))
...
```{r ordination plot of all data, eval=FALSE, fig.cap="\\label{fig:fig 8}Figure 8: Ordination on Jaccard distances for the complete dataset for both locations. Orange is Florida, fig.width=4, message=FALSE, warning=FALSE, blue is Monterey. This figure is not for publishing, I made it to see how different the communities are between the locations.", fig.height=4, include=FALSE}
